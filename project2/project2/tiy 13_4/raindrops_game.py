...
class RaindropsGame:
    ...
    def _update_raindrops(self):
        """Update drop positions, and look for dropsthat have disappeared. """
        self.raindrops.update()

        # Making new drops mean...
        make_new_drops = False
        for drop in self.raindrops.copy():
            if drop.check_disappeared():
                # Removing this will means creating new drops.
                self.raindrops.remove(drop)
                make_new_drops = True

        # Make new row of drops if needed.
        if make_new_drops:
            self._create_row(0)
...
if __name__ == '__main__':
    # Make a game example and run the game.
